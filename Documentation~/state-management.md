...
App UI offers two type of `Store` classes:
- A generic type [Store&lt;TStore&gt;](xref:Unity.AppUI.Redux.Store`1)
- A non-generic type [Store](xref:Unity.AppUI.Redux.Store) which is a specialization of the generic type.
...
If you want to support slices in your state, you can use the non generic version of the `Store` class.
Or you can implement the [IPartionableState](xref:Unity.AppUI.Redux.IPartionableState`1) interface in your state object.

Example of using the non generic version of the `Store` class:
```cs
using Unity.AppUI.Redux;
...
// Create an AsyncThunkActionCreator that will be used to dispatch the long operation.
...
{
    // This Enhancer will work for both the Store<TStoreState> type and the non-generic Store type.
    public StoreEnhancer<TStore,TStoreState> LoggerEnhancer<TStore,TStoreState>()
...
The [DevTools](xref:Unity.AppUI.Redux.DevTools) class is
an enhancer that can be used to add a time-traveling debugger to the store. The DevTools is still in development and
is not yet available in the current version of App UI.
